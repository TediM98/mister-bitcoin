...
//     contacts = utilService.loadFromStorage(CONTACT_KEY)
  ...
  return storageService.query(CONTACT_KEY)
    .then(contacts => {
      ...
      if (gSortBy.balance !== undefined) {
        contacts.sort((c1, c2) => (c1.balance - c2.balance) * gSortBy.balance)
      } else if (gSortBy.name !== undefined) {
        contacts.sort((c1, c2) => c1.name.localeCompare(c2.name) * gSortBy.name)
      }
      return contacts
    })
}
...
function getNextContactId(contactId) {
  return storageService.query(CONTACT_KEY)
    .then(contacts => {
      var idx = contacts.findIndex(contact => contact.id === contactId)
